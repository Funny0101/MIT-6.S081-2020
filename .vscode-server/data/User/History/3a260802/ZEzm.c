...
sys_mmap(void)
{
  ...
  // 获取外界参数并存储在定义的变量中 如果发生异常则退出
  if(argaddr(0, &addr)<0 || argint(1, &len)<0
    || argint(2, &prot)<0 || argint(3, &flags)<0
    || argfd(4, &fd, &file)<0 || argint(5, &offset)<0)
    return -1;

  /** 文件不可写 但映射在memmory中的区域是可写的
      并且规定该区域修改后应该更新文件本体 */
  ...
  // Caller在addr处填0意味着选择空闲区域的权利交给kernel了，
  // kernel可以根据进程当前所使用的内存情况进行分配；
  // 反之按Caller规定的地址来
  if(addr == 0) 
    vma->addr = p->sz;
...
// NEW
// 根据 addr 在虚拟内存中释放映射区域
// 根据 addr 锁定映射区域 然后逐一释放文件映射在虚拟内存中的 pages
// Caller 的 flags 是 MAP_SHARED 将 in-memory 中的更新内容回写至 disk 中
uint64
sys_munmap(void)
{
  uint64 addr;
