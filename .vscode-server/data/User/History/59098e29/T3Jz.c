...
found:
  ...
  if(pa == 0)
    panic("kalloc");
  uint64 va = KSTACK((int) (p - proc));
  ukvmmap(p->kernelPagetable,va, (uint64)pa, PGSIZE, PTE_R | PTE_W);
  p->kstack = va;
...
userinit(void)
{
  ...
  // NEW
  uvmcopy_not_physical(p->pagetable, p->kernelPagetable, 0, p->sz);
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    uvmcopy_not_physical(p->pagetable, p->kernelPagetable, p->sz, sz);
  } 
  else if(n < 0){
    ...
    if(PGROUNDDOWN(newsz) < PGROUNDUP(p->sz))
    {
      ...
      uvmunmap(p->kernelPagetable, PGROUNDUP(newsz), npages, 0);
    }
...
fork(void)
{
  ...
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0 || uvmcopy_not_physical(np->pagetable, np->kernelPagetable, 0, p->sz) < 0){
    freeproc(np);
    release(&np->lock);
    return -1;
  }
