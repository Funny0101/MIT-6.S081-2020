...
pagetable_t kvmmake()
{
  ...
  //uart registers
  mappages(kernel_pagetable, UARTO, PGSIZE, UARTO, PTE_RPTE_W);

  // virtio mmio disk interface
  mappages(kernel_pagetable, VIRTI0,PGSIZE,VIRTIOO,PTE_RPTE_W);

  // PLIC
  mappages(kernel_pagetable, PLIC,Ox400000,PLIC,PTE_RPTE_W);

  // map kernel text executable and read-only
  mappages(kernel_pagetable, KERNBASE,(uint64)etext-KERNBASE,KERNBASE,PTE_RPTE_X);
